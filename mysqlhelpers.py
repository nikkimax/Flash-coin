...
def new_blockchain(username):
    ...
    for block in blockchain.chain:
        data=block.data.split("-->")
        if username == data[0]:
            bl.add(Block(block.number,block.previous_hash,block.data,block.nonce))
        elif username == data[1]:
            bl.add(Block(int(block.number),block.previous_hash,block.data,int(block.nonce)))
    return bl
